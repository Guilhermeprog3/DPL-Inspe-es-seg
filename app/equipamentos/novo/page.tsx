...
function SelectOuOutro({
  options, value, valorManual, onChange, onChangeManual, placeholder, placeholderManual,
}: {
...
export default function NovoEquipamentoPage() {
  ...
  // Nº de Série — usado apenas para não-extintores (extintor usa serieCilindro como código)
  ...
  // O "código" enviado ao backend: cilindro para extintor, numeroSerie para os demais
  ...
  // Para extintor, o campo obrigatório de identificação é o serieCilindro
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (res.status === 401) {
        alert('Não autorizado. Por favor, refaça o login.')
        router.push('/login')
        return
      }
      if (!res.ok) {
      ...
      setNovoId(result.codigo || codigoFinal)
      setSuccessModal(true)
    } catch (error: any) {
      alert(error.message || 'Erro ao cadastrar. Verifique se o Nº de Série já existe.')
    } finally {
      setIsSaving(false)
    }
  }
      ...
      <div className="w-full flex flex-col bg-[#f4f6f9] min-h-[calc(100vh-60px)]">
        ...
        <div className="bg-white border-b border-[#e3e8ef] px-7 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2 text-[13px] text-[#9ca3af]">
            ...
            <span className="text-[#3d6cf0] font-semibold">Novo Cadastro</span>
          </div>
          ...
          {tab === 'identificacao' && (
            <div className="fade-up space-y-4">
                ...
                <div className="p-6">
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-4">
                    {TIPO_OPTIONS.map(t => {
                      ...
                      return (
                        <button key={t.value}
                          onClick={() => { setTipo(t.value); setIsTipoManual(false) }}
                          className={cn(
                    ...
                    {/* Outro */}
                    <button
                      onClick={() => { setIsTipoManual(true); setTipo('') }}
                      className={cn(
                        'flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all border-dashed',
                  ...
                  {isTipoManual && (
                    <div className="slide-down">
                      <input className={cn(inp, 'border-dashed border-[#3d6cf0] bg-blue-50/30')}
                        placeholder="Especifique o tipo do equipamento..."
                        value={tipoManual} onChange={e => setTipoManual(e.target.value)} autoFocus />
                    </div>
                  )}
                </div>
              </div>

              {/* Extintor — campos técnicos */}
              {isExtintor && (
                <div className="slide-down bg-white border-2 border-[#bfdbfe] rounded-xl shadow-sm overflow-hidden">
                  ...
                  <div className={cn('px-6', divi)}>
                    <div className={row}>
                      <div>
                        <p className={lbl}>Classe *</p>
                        <p className={lbl2}>Tipo de incêndio combatido</p>
                      </div>
                      <SelectOuOutro
                        options={EXTINTOR_CLASSE_OPTIONS}
                        value={extClasse} valorManual={extClasseManual}
                        onChange={setExtClasse} onChangeManual={setExtClasseManual}
                        placeholder="Selecione a classe..."
                        placeholderManual="Ex: ABCDK, especial..." />
                    </div>

                    <div className={row}>
                      <div>
                        <p className={lbl}>Carga *</p>
                        <p className={lbl2}>Peso do agente extintor</p>
                      </div>
                      <div className="flex items-center gap-2 max-w-[180px]">
                        <input type="number" min="1" step="0.5" value={extCarga}
                          onChange={e => setExtCarga(e.target.value)}
                          className={inp} placeholder="Ex: 4, 6, 12..." />
                        <span className="text-sm font-bold text-slate-500 whitespace-nowrap">kg</span>
                      </div>
                    </div>

                    <div className={row}>
                      <div>
                        <p className={lbl}>Agente Extintor *</p>
                        <p className={lbl2}>Substância utilizada</p>
                      </div>
                      <SelectOuOutro
                        options={EXTINTOR_AGENTE_OPTIONS}
                        value={extAgente} valorManual={extAgenteManual}
                        onChange={setExtAgente} onChangeManual={setExtAgenteManual}
                        placeholder="Selecione o agente..."
                        placeholderManual="Ex: Água + Espuma, AFFF..." />
                    </div>

                    <div className={row}>
                      <div className="flex items-start gap-2">
                        <CalendarClock size={15} className="text-[#1d4ed8] mt-0.5 flex-shrink-0" />
                        <div>
                          <p className={lbl}>Validade da Recarga</p>
                          <p className={lbl2}>Data da próxima recarga obrigatória</p>
                        </div>
                      </div>
                      <input type="date" value={validadeRecarga}
                        onChange={e => setValidadeRecarga(e.target.value)}
                        className={cn(inp, 'max-w-[200px]')} />
                    </div>
                  </div>

                  {(extClasse && extClasse !== 'outro' || extClasseManual) && extCarga &&
                   (extAgente && extAgente !== 'outro' || extAgenteManual) && (
                    <div className="px-6 py-3 bg-blue-50/60 border-t border-[#bfdbfe] flex items-center gap-3">
                      <div className="w-7 h-7 bg-[#1d4ed8] rounded-lg flex items-center justify-center flex-shrink-0">
                        <Flame size={13} color="#fff" />
                      </div>
                      <p className="text-[12px] font-bold text-[#1d4ed8]">
                        Extintor {extClasse === 'outro' ? extClasseManual : extClasse}
                        {' · '}{extCarga} kg
                        {' · '}{extAgente === 'outro' ? extAgenteManual : extAgente}
                        {validadeRecarga && ` · Recarga: ${new Date(validadeRecarga + 'T00:00:00').toLocaleDateString('pt-BR')}`}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="bg-white border border-[#e3e8ef] rounded-xl shadow-sm overflow-hidden">
                ...
                <div className={cn('px-6', divi)}>
                  <div className={row}>
                    <span className={lbl}>Nome / Descrição *</span>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {STATUS_OPTIONS.map(opt => (
                        <button key={opt.value} onClick={() => setStatus(opt.value)}
                          className={cn(
                            'px-4 py-2 rounded-lg border text-xs font-bold transition-all flex items-center gap-1.5',
              ...
              <div className="bg-white border border-[#e3e8ef] rounded-xl shadow-sm overflow-hidden">
                ...
                <div className={cn('px-6', divi)}>
                  ...
                  {/* Nº de Série / Patrimônio — apenas para não-extintores */}
                  {!isExtintor && (
                    <div className={row}>
                      <div className="flex items-start gap-2">
                        <Hash size={15} className="text-slate-400 mt-0.5 flex-shrink-0" />
                        <div>
                          <p className={lbl}>Nº de Série / Patrimônio *</p>
                          <p className={lbl2}>Número do fabricante / Código de Barras</p>
                        </div>
                      </div>
                      <input type="text" value={numeroSerie} onChange={e => setNumeroSerie(e.target.value)}
                        className={inp} placeholder="Ex: AMX-2024-9921" />
                    </div>
                  )}

                  {/* Campos exclusivos do extintor */}
                  {isExtintor && (
                    <>
                      <div className="slide-down">
                        <div className={row}>
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <p className={lbl}>Nº do Selo INMETRO</p>
                              <p className={lbl2}>Número de série do lacre/selo</p>
                            </div>
                          </div>
                          <input type="text" value={serieInmetro} onChange={e => setSerieInmetro(e.target.value)}
                            className={inp} placeholder="Ex: 0123456789" />
                        </div>
                      </div>
                      ...
                      <div className="slide-down">
                        <div className={row}>
                          <div className="flex items-start gap-2">
                            <Package size={15} className="text-slate-500 mt-0.5 flex-shrink-0" />
                            <div>
                              <p className={lbl}>Nº de Série do Cilindro *</p>
                              <p className={lbl2}>Gravado no corpo metálico</p>
                            </div>
                          </div>
                          <input type="text" value={serieCilindro} onChange={e => setSerieCilindro(e.target.value)}
                            className={inp} placeholder="Ex: CIL-2022-00412" />
                        </div>
                      </div>
                    </>
                  )}
                ...
                <div className="p-6">
                  <div className="flex items-start gap-3 p-4 bg-blue-50 border border-blue-100 rounded-xl mb-6">
                    ...
                    <p className="text-sm text-blue-700">
                      Selecione o ponto físico (QR Code) onde este equipamento está instalado.
                      Deixe em branco se estiver em estoque.
                    </p>
                  </div>
                  <div className="grid gap-4 items-center grid-cols-1 sm:grid-cols-[220px_1fr]">
                    ...
                    <div className="relative">
                      <select className={cn(inp, 'appearance-none pr-8')}
                        value={pontoId || ''} onChange={e => setPontoId(e.target.value || null)}>
                        <option value="">NÃO VINCULAR (EM ESTOQUE)</option>
                        {pontosDisponiveis.map(p => (
                          <option key={p.id} value={p.id}>QR: {p.qrCode} - {p.nome}</option>
                        ))}
                      </select>
                      <ChevronDown size={14} className="absolute right-3 top-3 text-slate-400 pointer-events-none" />
                    </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md border-t px-8 py-4 flex items-center justify-between z-50">
          <div className="hidden md:flex items-center gap-4">
            <div className="flex gap-1.5">
              {tabOrder.map(key => (
                <div key={key} className={cn('h-1.5 w-6 rounded-full transition-all', tabValid[key] ? 'bg-[#3d6cf0]' : 'bg-slate-200')} />
              ))}
            </div>
          ...
          <div className="flex gap-3 w-full md:w-auto">
            {currentIdx > 0 && (
              <button onClick={() => setTab(tabOrder[currentIdx - 1])}
        ...
        {successModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-6">
            <div className="bg-white p-10 rounded-3xl text-center shadow-2xl max-w-sm scale-in">
              ...
              <p className="text-slate-500 text-sm mb-8">Equipamento registrado com sucesso no inventário.</p>
