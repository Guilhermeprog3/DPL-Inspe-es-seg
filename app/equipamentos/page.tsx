...
export default function EquipamentosPage() {
  ...
  const equipamentosFiltrados = useMemo(() => {
    return (mockEquipamentos as any[]).filter((e) => {
  ...
  const selectedEquipamentos = (mockEquipamentos as any[]).filter(e => selectedIds.includes(e.id))
    ...
    <DashboardLayout title="Equipamentos" breadcrumb="SIGS / Inventário">
      ...
      <style>{`
        ...
        @media print {
          ...
          .etiqueta-single {
            ...
            border: 0.2mm dashed #ccc !important; /* Guia de corte */
          ...
          .no-print, button, .bg-black/50 { display: none !important; }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select className="h-10 text-xs" value={filtros.base} onChange={(e) => setFiltros({...filtros, base: e.target.value})}>
                ...
                {Array.from(new Set(mockEquipamentos.map(m => m.base))).map(b => (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Novo Equipamento">
        <form className="space-y-5 pt-2" onSubmit={handleSubmit((data) => console.log(data))}>
          ...
          {isExtintor && (
            <div className="bg-blue-50/30 p-4 rounded-xl border border-blue-100/50 space-y-4 animate-in fade-in slide-in-from-top-1 duration-200">
      ...
      {/* Modal Impressão com Fontes Aumentadas */}
      <Modal open={qrModalOpen} onClose={() => setQrModalOpen(false)} title="Gerar Etiquetas">
        <div className="flex flex-col items-center">
          <div 
            id="print-area" 
            className="flex flex-wrap justify-center gap-4 max-h-[60vh] md:max-h-none overflow-y-auto p-4 bg-gray-50 rounded-xl"
          >
            {selectedEquipamentos.map((eq) => (
              <div key={eq.id} className="etiqueta-single border-[1.5px] border-[#dde3ec] p-10 text-center bg-white">
                ...
                <p className="text-[42px] font-black text-[#0d1e33] mt-8 uppercase leading-none tracking-tight">
                  {eq.codigo}
                </p>
                <p className="text-[20px] font-bold text-[#094780] uppercase mt-3 tracking-wide">
                  {eq.tipo}
                </p>
                
                <div className="w-full pt-6 mt-8 border-t-[0.5mm] border-[#f1f5f9]">
                   <p className="text-[22px] font-black uppercase text-gray-800 leading-tight">
                     {eq.base}
                   </p>
          ...
          <div className="mt-8 w-full space-y-2 no-print">
            <Button className="w-full h-12 gap-2" onClick={() => window.print()}>
              <Printer size={18} /> Imprimir Etiquetas ({selectedIds.length})
            </Button>
            <Button variant="ghost" className="w-full text-[#6b7a90]" onClick={() => setQrModalOpen(false)}>
              Cancelar
            </Button>
          </div>
        </div>
